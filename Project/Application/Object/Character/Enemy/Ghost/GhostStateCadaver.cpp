...
void GhostStateCadaver::Initialize()
{
	...
	*colliderShape = obb;
	...
	for (uint32_t i = 0; i < structuralSpring_.size(); ++i) {
		...
		if (childerenSize == 0) {
			Vector3 add = Matrix4x4::TransformNormal({0.0f,0.0f,1.0f}, localMatrixManager->GetNodeDatas()[kExtraMatrixNum + i].matrix);
			postion1 = postion0 + add;
		}
		else {
		...
		//naturalLength
		Vector3 length = postion0 - postion1;
		float naturalLength = Vector3::Length(length);
	...
	MassPoint massPointInitVelocity = structuralSpring_[0].GetPoint0();
	massPointInitVelocity.velocity = { 0.0f, 2.0f,0.0f };
	structuralSpring_[0].SetPoint0(massPointInitVelocity);
